...
import { Navbar, Content } from 'ionic-angular';
...
@IonicPage()
@Component({
  selector: 'page-sale-submit',
  templateUrl: 'sale-submit.html',
})
export class SaleSubmitPage {
  ...
  public phoneScreen:boolean=true;
  constructor(
    ...
    this.code = false;
    this.phoneScreen=navParams.get('phone')?false:true;
...
// 读取本地缓存 并且禁止ios 右滑动
      ...
      buttons: [
        {
          ...
          handler: data => {
            ...
            this.appservice.httpPost('submitCasPosSale.api', {
              ...
              if (data.code == 1) {
                this.navCtrl.push('SaleInputPage', { data: data['data'] })
  ...
  caculator(index,num){
   ...
   if(num>=0){
    this.goods[index]['num']=parseInt(num);
   }else{
